# Spec: form-control-migration

> Artifact store: hybrid. Implemented by change `ep-extraction-v3`. Branches: `feat/ds-ep-extraction-v3-wu3`, `feat/ds-ep-extraction-v3-wu5`.
> This spec documents the migration of 5 form-control packages off `element-plus` internal imports, as implemented and verified.

## Purpose

Migrate 5 form-control component packages — `checkbox`, `radio`, `switch`, `segmented`, `input` — off `element-plus` internal imports onto the design system's own hooks/utils (`g-utils`, `g-hooks`, `g-form`), without changing any package's public API. `input-number` is explicitly excluded from this change (deferred to `ep-extraction-v4`).
...
- GIVEN the migration usage audit finds `ValidateComponentsMap` imported but never referenced in `input.vue`
- WHEN the migration is completed
- THEN the import is removed entirely and no local `ValidateComponentsMap` file is created

**As implemented**: the usage audit (WU-5) confirmed `ValidateComponentsMap` appeared only on its import line in `input.vue` with zero call sites — the import was dropped entirely; no local `ValidateComponentsMap` file was created.
...
### Requirement: gform-disabled-cascade-behavior-pinned

The `<GForm disabled>` → child control disabled-state cascade (via `useFormDisabled`) MUST be covered by a genuine regression test asserting real component mount + real `g-form` `formContextKey` injection, not a tautological/smoke-only assertion. This behavior was previously a silent no-op before migration (the cascade existed in element-plus's own implementation but was not exercised/pinned in this design system's test suite).

#### Scenario: form-level disabled cascades to child control

- GIVEN a `GForm` with `disabled: true` wrapping a `GCheckbox` with no explicit `disabled` prop
- WHEN the checkbox's effective disabled state is read via `useFormDisabled`
- THEN it resolves to `true`, cascaded from the form context
...
- Depends on: `openspec/specs/g-utils-extended/spec.md`, `openspec/specs/g-hooks-package/spec.md`, `openspec/specs/g-form/spec.md`
