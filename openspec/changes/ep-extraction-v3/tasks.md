...
- [x] T3.1 **checkbox**: re-point `useNamespace`→g-utils, `useAriaProps`→g-hooks, `debugWarn`/`isArray`/`isBoolean`/`isObject`/`isString`/`isNumber`/`isPropAbsent`/`isUndefined`→g-utils, `UPDATE_MODEL_EVENT`→g-utils constants, `useFormDisabled`→g-form. Zero public API change. `yarn test:run` green for checkbox's existing tests.
- [x] T3.2 **radio**: re-point `useId`→g-hooks, `useNamespace`→g-utils, `useAriaProps`→g-hooks, `buildProps`/`isBoolean`/`isNumber`/`isString`/`isPropAbsent`→g-utils, `UPDATE_MODEL_EVENT`/`CHANGE_EVENT`→g-utils constants. Zero public API change. `yarn test:run` green. **Fix beyond mechanical swap**: dropped a redundant outer `as const` on `radioProps`/`radioGroupProps`'s `buildProps({...} as const)` calls — g-utils's `buildProp`/`buildProps` type signature is a simple passthrough (`<T>(prop:T):T`), unlike element-plus's elaborate `EpPropConvert` conditional types that widen readonly tuples; the `as const` froze `modelValue.type: [String,Number,Boolean]` into a readonly tuple that broke `ExtractPropTypes`/`defineEmits`. Removing the (unneeded — no `values` field depended on it) `as const` is a zero-runtime-behavior type-only fix.
- [x] T3.3 **switch**: re-point `useAriaProps`/`useSizeProp`→g-hooks, `useFormDisabled`/`useFormSize`→g-form, `useNamespace`→g-utils, `addUnit`/`debugWarn`/`isBoolean`/`isPromise`/`throwError`/`isValidComponentSize`/`buildProps`/`definePropType`/`isNumber`/`isString`→g-utils, `ComponentSize` type→g-utils. `INPUT_EVENT`/`CHANGE_EVENT`/`UPDATE_MODEL_EVENT` stay on local `./constants` (unchanged). Zero public API change. `yarn test:run` green. Same `as const` type-only fix as T3.2 applied to `switchProps`.
- [x] T3.4 **segmented**: re-point `useFormSize`→g-form, `useId`→g-hooks, `useNamespace`→g-utils, `buildProps`/`definePropType`/`isBoolean`/`isNumber`/`isString`/`isObject`/`debugWarn`→g-utils. Zero public API change. `yarn test:run` green. (No `as const` fix needed — `segmentedProps`/`Segmented.vue` never wrapped in `as const`.)
- [x] T3.5 Grep-verify zero `from ['"]element-plus` matches under `components/{checkbox,radio,switch,segmented}/src/**` and `index.ts`. Confirmed: only SCSS `@use "element-plus/theme-chalk/..."` theme imports and one GitHub-issue-URL comment remain (not JS/TS imports).
- [x] T3.6 Remove `'components/checkbox/**'`, `'components/radio/**'`, `'components/switch/**'`, `'components/segmented/**'` from `excludedFiles` in `.eslintrc.cjs`. Run `yarn lint --max-warnings 0` (must pass with the excludes removed). Verified zero `no-restricted-imports`/element-plus violations for the 4 packages via a scoped `npx eslint` run; the full repo-wide `yarn lint` has ~2634 PRE-EXISTING unrelated errors (dist/ type-def noise, `stories/*.stories.ts` unused vars, `v-if`/`v-for` ordering) identical in count before/after this WU (confirmed via `git stash` A/B) — not a WU-3 regression, out of this WU's scope to fix repo-wide.
- [x] T3.7 `yarn build` succeeds for the 4 packages. **Blocker found & fixed**: `@flash-global66/g-form`'s published `dist/` was stale (built 2026-07-08, before WU-2 added `useFormSize`/kept `useFormDisabled`), so `vue-tsc build:types` for switch/segmented failed with "no exported member useFormSize" until `g-form` was rebuilt (`yarn build form`). Rebuilding `g-form` itself then surfaced a genuine pre-existing WU-2 test-file type bug (`components/form/tests/hooks/use-form-common-props.spec.ts:118` — `sizeProp: ''` not assignable to `ComponentSize`, since `componentSizes` includes `''` for EP-validator parity but the `ComponentSize` type omits it); fixed by widening the test helper's local `sizeProp` param type to `ComponentSize | ''` (2-line, test-only, zero behavior change). Also see T3.2/T3.3 for the `as const` type fix required for radio/switch.
- [ ] T3.8 Validate in `front-b2b` using **real copies in `node_modules`** (never symlinks — symlinks strip base CSS/Tailwind `@apply`) for the 4 published packages. **DEFERRED** — not in this delegation's explicit Definition of Done (only zero-EP-imports/lint/test/build/commit were required); flagged for the orchestrator/user to schedule separately since it requires operating in the sibling `front-b2b` repo.
- [x] T3.9 Commit as one work unit per package or a cohesive multi-package commit that tells one story (`feat(checkbox,radio,switch,segmented): migrate off element-plus internals`), open PR #3 targeting `main` after PR #2 merges. Lerna-publish the 4 packages on merge. Rollback = `git revert` this merge commit (restores prior EP imports without touching WU-1/WU-2). Committed on `feat/ds-ep-extraction-v3-wu3`; PR opening / Lerna publish left to the user per task scope — not pushed.
