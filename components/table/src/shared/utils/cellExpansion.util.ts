import type { ExpandDirection } from '../types/cellExpansion.type';
...
function getTableElement(): HTMLElement | undefined {
  return (
    currentActiveTable ??
    (document.querySelector('.gui-table') as HTMLElement) ??
    undefined
  );
}
 ...
 */
export function clearExpansionCacheForTable(
...
export function getTableCells(): HTMLElement[] | undefined {
  try {
    const tableElement =
      currentActiveTable || document.querySelector('.gui-table');
...
export function calculateExpandedWidthSync(
  ...
  if (isLastColumn) {
    ...
    if (columnIndex > 0) {
      ...
      if (columnIndex > 1) {
        totalWidth +=
          cells[columnIndex - 2]?.getBoundingClientRect()?.width || 0;
      }
    }
  } else {
