...
import type { RenderCellData } from '../../../shared/types/cellRenderer.type';
...
function createTextareaEditVNode(
  ...
  return h(
    ...
    [
      ...
      isValidating
        ? h('span', {
            class: 'gui-table-cell-input-textarea-loading',
            'aria-hidden': 'true',
          })
        : null,
      h('textarea', {
      ...
      messageError
        ? h('div', { class: 'gui-table-cell-input-error' }, messageError)
        : null,
    ].filter(Boolean),
  );
}
...
function createReadVNode(config: InputCellConfig): VNode {
  ...
  const errorReadClass = isValidationError
    ? 'gui-table-cell-input-read--error'
    : '';
