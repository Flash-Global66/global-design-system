...
import { useInputCellState } from '../composables/useInputCellState';
...
function computeExpansionMetrics(
  ...
  if (config.expandedWidthOption) {
    width =
      typeof config.expandedWidthOption === 'function'
        ? config.expandedWidthOption(config.row, config.prop, config.idx)
        : config.expandedWidthOption;
  } else {
    width = calculateExpandedWidthSync(
      config.prop,
      config.colIndex,
      cacheKey,
      config.expandColspan,
      config.expandDirection,
    );
  }
  ...
  if (config.leftOffsetOption) {
    leftOffset =
      typeof config.leftOffsetOption === 'function'
...
function createTextareaEditVNode(
  ...
  return h(
    'div',
    { class: `${inputWrapperClass} ${ns.b('cell-input-textarea-wrapper')}` },
    [
      label ? h('label', { class: ns.b('cell-input-label') }, label) : null,
      isValidating
        ? h('span', {
            class: ns.b('cell-input-textarea-loading'),
            'aria-hidden': 'true',
          })
        : null,
      h('textarea', {
        class: `${ns.b('cell-input-textarea')} ${isValidationError ? ns.bm('cell-input-textarea', 'error') : ''} ${isValidating ? ns.bm('cell-input-textarea', 'validating') : ''}`,
        rows: rows,
        value: displayValue,
        placeholder,
      ...
      messageError
        ? h('div', { class: ns.b('cell-input-error') }, messageError)
        : null,
    ].filter(Boolean),
  );
}
...
function createInputEditVNode(
  ...
  return h('div', { class: inputWrapperClass }, [
    h(GInput as unknown as string, {
      ...
      messageError,
      class: `w-full ${isValidationError ? ns.bm('cell-input', 'error') : ''}`,
      'aria-invalid': isValidationError,
...
function createReadVNode(config: InputCellConfig): VNode {
  const {
    displayValue,
    emptyActionText,
    isEmpty,
  ...
  const errorReadClass = isValidationError
    ? ns.bm('cell-input-read', 'error')
    : '';
  ...
  return h(
    ...
    [
      h(
        ...
        {
          class:
            'absolute top-xs right-xs opacity-0 group-hover:opacity-100 transition-opacity duration-150',
        },
        [
          h(
            'span',
            {
              class:
                'group/badge inline-flex items-center justify-center p-0.5 rounded-sm bg-primary-bg border border-primary-bd hover:border-secondary-bd transition-colors duration-150 cursor-pointer',
            },
            [
              h(
                'span',
                {
                  class:
                    'px-1 rounded-sm hover:bg-sec-hover-bg transition-colors duration-150',
                },
                [
                  h(GIconFont as unknown as string, {
                    name: 'regular pen',
                    class:
                      'text-icon-primary group-hover/badge:text-icon-secondary leading-none',
                    style: { fontSize: '10px' },
                    'aria-hidden': true,
                  }),
                ],
      ...
      h(
        'div',
        { class: 'flex flex-col items-center w-full min-w-0 overflow-hidden' },
        [
          ...
          isValidationError && validationMessage && !isEditing
            ? h(
                'span',
                { class: 'text-error-def text-2 mt-xxs text-center px-xs' },
                validationMessage,
              )
            : null,
        ].filter(Boolean),
...
function computeWrapperClasses(
  ...
  let wrapperClass = config.isEditing
    ? `${cellWrapperClass} group ${ns.b('cell-input-wrapper')} ${ns.b('cell-input-editing')} hover:bg-everBlue-100 hover:bg-opacity-30`
    : `${cellWrapperClass} group ${ns.b('cell-input-wrapper')} ${ns.b('cell-input-reading')} hover:bg-everBlue-100 hover:bg-opacity-30`;

  if (config.isValidationError) {
    wrapperClass = `${wrapperClass} ${ns.bm('cell-input-wrapper', 'error')}`;
  }
...
export function renderInputCell(
  ...
  const columnEmptyActionText = (column as unknown as Record<string, unknown>)
    .emptyActionText as string | undefined;
  ...
  if (
    co &&
    typeof co.getEditing === 'function' &&
    typeof co.toggle === 'function' &&
    typeof co.setEditing === 'function'
  ) {
    getEditing = co.getEditing as (
      r: unknown,
      p: string,
      i?: number,
    ) => boolean;
    ...
    const getKey = (r: unknown, p: string, i?: number): string =>
      `${i ?? -1}-${p}`;
    getEditing = (r: unknown, p: string, i?: number) =>
      defaultEditingState.value === getKey(r, p, i);
    toggle = (r: unknown, p: string, i?: number) => {
      const key = getKey(r, p, i);
      defaultEditingState.value =
        defaultEditingState.value === key ? null : key;
    };
  ...
  const hasValidation =
    validation && validation.rules && validation.rules.length > 0;
  const validationState = hasValidation
    ? validation.getValidationState(row, prop, idx)
    : null;
  ...
  const columnInputLabel = (column as unknown as Record<string, unknown>)
  ...
  const emptyActionText =
    (co?.emptyActionText as string) ??
    columnEmptyActionText ??
    placeholder ??
    'Click to add';
  const type = (co?.type as string) ?? 'text';
  const rows = co?.rows as number | undefined;
  const formatter = co?.formatter as
    | ((value: string | number, ...args: unknown[]) => string)
    | undefined;
  const parser = co?.parser as
    | ((value: string, ...args: unknown[]) => string | number)
    | undefined;
  const staticMessageError = co?.messageError as string | undefined;
  const messageError = isValidationError
    ? validationMessage
    : staticMessageError;
  const isEmpty = !displayValue || displayValue.trim() === '';

  const expandColspanOption = co?.expandColspan as number | undefined;
  const columnExpandColspan = (column as unknown as Record<string, unknown>)
    .expandColspan as number | string | undefined;
  const expandColspan =
    expandColspanOption ??
    (columnExpandColspan ? Number(columnExpandColspan) : undefined);
  const expandDirectionOption = co?.expandDirection as
    | 'left'
    | 'right'
    | undefined;
  const columnExpandDirection = (column as unknown as Record<string, unknown>)
    .expandDirection as string | undefined;
  const expandDirection =
    expandDirectionOption ??
    (columnExpandDirection === 'left' || columnExpandDirection === 'right'
      ? columnExpandDirection
      : undefined);
  ...
  const config: InputCellConfig = {
    ...
    expandedWidthOption: co?.expandedWidth as
      ...
      | undefined,
    leftOffsetOption: co?.leftOffset as
      | number
      | ((row: unknown, prop: string, index?: number) => number | undefined)
      | undefined,
    editingClasses: co?.editingClasses as
      | string
      ...
      | undefined,
    editingStyles: co?.editingStyles as
      | Record<string, string>
  ...
  const { width: computedExpandedWidth, leftOffset: computedLeftOffset } =
    computeExpansionMetrics(config, cacheKey);

  const computedEditingClasses =
    isEditing && config.editingClasses
      ? typeof config.editingClasses === 'function'
        ? config.editingClasses(row, prop, idx)
        : config.editingClasses
      : undefined;
  const computedEditingStyles =
    isEditing && config.editingStyles
      ? typeof config.editingStyles === 'function'
        ? config.editingStyles(row, prop, idx)
        : config.editingStyles
      : undefined;
  ...
  const handleBlur = async () => {
    if (hasValidation && validation) {
      ...
      if (table?.emit && column) {
        table.emit('cell-edit-validate', row, column, {
          valid: result.valid,
          message: result.message,
        });
      }
  ...
  const cellWrapperClass =
    isEditing && rows !== undefined && rows > 0
  ...
  const inputWrapperClass =
    computedExpandedWidth !== undefined ? 'w-full px-xs' : 'w-11/12 px-xs';
  ...
  const wrapperClass = computeWrapperClasses(
    config,
    cellWrapperClass,
    computedEditingClasses,
  );
  const wrapperStyle = computeWrapperStyles(
    config,
    computedExpandedWidth,
    computedLeftOffset,
    computedEditingStyles,
  );
  ...
  return h(
    ...
    [
      h(
        Transition,
        { name: ns.b('cell-edit'), mode: 'out-in' },
