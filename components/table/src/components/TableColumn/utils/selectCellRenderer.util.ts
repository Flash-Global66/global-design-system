...
export function renderSelectCell(
  ...
  const getOptionsFn = co?.getOptions as
    | ((row: unknown) => SelectOption[])
    | undefined;
  const opts = getOptionsFn
    ? getOptionsFn(data.row)
    : (co?.options as SelectOption[]) || [];
  const getEditing = co?.getEditing as
    | ((r: unknown, p: string, i?: number) => boolean)
    | undefined;
  const toggle = co?.toggle as
    | ((r: unknown, p: string, i?: number) => void)
    | undefined;
  const setEditing = co?.setEditing as
  ...
  const emptyActionText =
    (co?.emptyActionText as string) ?? columnEmptyActionText ?? '';
  const isEmpty =
    val === undefined ||
    val === null ||
  ...
  if (getEditing && toggle && setEditing) {
    ...
    if (isEditing) {
      ...
      if (expandedWidthOption) {
        ...
        computedExpandedWidth = calculateExpandedWidthSync(
          prop,
          colIndex,
          cacheKey,
          expandColspan,
      ...
      if (leftOffsetOption) {
        ...
        computedLeftOffset = calculateLeftOffset(
          colIndex,
          cacheKey,
          expandDirection,
          expandColspan,
    ...
    const editVNode = h(GSelect, {
      class: `${ns.b('cell-select')} ${ns.b('cell-select-editing')} w-full min-w-0 px-xs`,
      modelValue: val,
      label: fieldLabel,
      ...
      onVnodeMounted: (vnode: VNode) => {
        nextTick(() => {
          ...
          if (proxy && typeof proxy.focus === 'function') {
            ...
            const input =
              el.querySelector('input') || el.querySelector('select') || el;
    ...
    const readVNode = h(
      'div',
      {
        class: `${ns.b('cell-select-read')} relative w-full h-full min-w-0 max-w-full flex items-center justify-start overflow-hidden px-xs`,
        role: 'button',
        tabIndex: 0,
        title: displayText,
      ...
      [
        h(
          ...
          {
            class:
              'absolute top-xxs right-xxs opacity-0 group-hover:opacity-100 transition-opacity duration-150',
          },
          [
            h(
              'span',
        ...
        h(
          'span',
          {
            class: `${ns.be('cell-select-read', 'label')} min-w-0 w-full flex-1 text-left text-pretty break-words leading-snug line-clamp-2`,
          },
          displayText,
        ),
      ],
    ...
    const wrapperClass = `group ${ns.b('cell-edit-wrapper')} absolute top-0 left-0 h-full w-full flex items-center justify-start transition-all duration-200 ease-in hover:bg-everBlue-100 hover:bg-opacity-30`;
    ...
    return h(
      ...
      [
        h(
          'div',
          {
            class: `${ns.b('cell-select-inner')} min-h-0 min-w-0 w-full h-full flex items-center justify-start`,
          },
          [
            h(
              Transition,
  ...
  return h(
    ...
    opts.map(o =>
      h(
        'option',
        { value: o.value, key: o.value },
        o.title ?? o.label ?? String(o.value),
      ),
    ),
  );
}
