...
import type { ComputedRef, Ref, SetupContext, WritableComputedRef } from 'vue';
...
type useSelectReturnType = (
  ...
  hasHelpInfo: ComputedRef<boolean>;
...
/**
 * Migración de element-plus (WU-8 `select`, ep-extraction-v4): se elimina
 * el `validateIcon` computed (y su dependencia de `ValidateComponentsMap` /
 * `@element-plus/icons-vue`) porque es código muerto — nunca se renderiza
 * en `select.vue` (el template solo usa `g-icon-font`), y portarlo obligaría
 * a introducir una dependencia nueva en el ecosistema de element-plus solo
 * para un valor inalcanzable. `validateState` (de donde `validateIcon` leía
 * la clave) se mantiene intacto, ya que no depende de element-plus.
 */
const useSelect: useSelectReturnType = (
  ...
  const isErrorKls = computed(() =>
    Boolean(selectError.value || elFormItem?.shouldShowError),
  );
  ...
  const hasHelpInfo = computed(() => {
    return Boolean(elFormItem?.$el || selectError.value || props.helpText);
  });
  ...
  const filterOptions = (query: string) => {
    ...
    return [...states.createdOptions, ...props.options].reduce((all, item) => {
      ...
      if (isArray(options)) {
        // g-utils' `isArray` narrowea a `unknown[]` (más estricto que el
        // `any[]` con el que tipa element-plus/@vue/shared su propio
        // `isArray`); se castea a `Option[]` porque `getOptions` siempre
        // retorna un array de opciones en este branch (grupo anidado).
        const filtered = (options as Option[]).filter(isValidOption);
  ...
  const labelStyle = computed(() => {
    ...
    return {
      left: !shouldMoveLabel
        ? `calc(${leftPrefixSelect.value} + 18px)`
        : undefined,
    };
  });
      ...
      if (hasModelValue.value) {
        ...
        if (~selectedItemIndex) {
          ...
          if (!states.selectedLabel || needUpdateSelectedLabel) {
            // g-utils' `isObject` narrowea a `Record<string, unknown>` (más
            // estricto que el `Record<any, any>` con el que tipa
            // element-plus/@vue/shared su propio `isObject`), lo que hace
            // que `getValueKey` retorne `unknown` en vez de `any` como antes
            // — mismo tipo de gap pre-existente que `isArray` (línea ~283).
            states.selectedLabel = getValueKey(props.modelValue) as string;
          }
        }
      } else {
        states.selectedLabel = '';
  ...
  watch(
    ...
    () => {
      ...
      const shouldResetScrollTop =
        props.filterable && states.inputValue.length > 0;
