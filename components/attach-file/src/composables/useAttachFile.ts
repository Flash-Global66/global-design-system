...
import type { UseAttachFileOptions } from "../attach-file.type";

export { UseAttachFileOptions };
...
export function useAttachFile(options: UseAttachFileOptions) {
  ...
  const isSingleFileView = computed(() => !unref(options.multiple) || (unref(options.files)?.length || 0) === 1);

  const isMultipleFilesView = computed(() => unref(options.multiple) && (unref(options.files)?.length || 0) > 1);

  const showInfoText = computed(
    () =>
      unref(options.multiple) ||
      (isError.value && (!unref(options.multiple) || (unref(options.files)?.length || 0) === 1))
  );

  const showFileList = computed(() => (unref(options.files)?.length || 0) > 0);
  ...
  function validateFileCount(currentFilesCount: number, newFilesCount: number, maxFiles?: number): string | null {
    if (maxFiles && currentFilesCount + newFilesCount > maxFiles) {
      ...
      return `Se permite un máximo de ${maxFiles} ${fileWord}. Intenta subir ${
  ...
  const safeFiles = computed(() => unref(options.files) || []);

  function getFileStatus(index: number): FileStatus {
    const statuses = unref(options.fileStatuses) || {};
    return statuses[index] || FILE_STATUS.SUCCESS;
  }

  function fileName(index: number, file: File): string {
    const errors = unref(options.fileErrors) || {};
    const error = errors[index];
    return error || file.name;
  }

  function createRemoveFile(
    emit: (event: "update:modelValue", files: File[]) => void,
    changeEmit: (event: "change", files: File[]) => void
  ) {
    return function removeFileAtIndex(index: number) {
      const currentFiles = unref(options.files) || [];
      const updatedFiles = [...currentFiles];
      updatedFiles.splice(index, 1);
      emit("update:modelValue", updatedFiles);
      changeEmit("change", updatedFiles);
    };
  }
